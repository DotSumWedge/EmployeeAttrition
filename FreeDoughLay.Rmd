...
for(i in 1:length(columnCombinations)){
  ...
  fileLocation = paste("C:/SMU MS Data Science/DoingDataScience6306/EmployeeAttrition/KnnResults/Combinations2/Results", i, ".csv", sep="")
...
#for(i in 1:length(columnCombinations)){
  resultsDataframe = data.frame(accuracy = double(), specificity = double(), kValue = integer(), combination = character())
  columnCombinationsMatrix = combn(columnCombinations, 2)
  #for(j in 1:ncol(columnCombinationsMatrix)){
    for(k in 1:maxKvalue){
      iterationResultsDataframe = data.frame(accuracy = double(), specificity = double(), kValue = integer(), combination = character())
      for(l in 1:iterations){
        ...
        classifications = knn.cv(trainingDataSet[,c(3,4,5,6)], trainingDataSet$Attrition, k=k, prob = TRUE)
        iterationResultsDataframe[nrow(iterationResultsDataframe) + 1,] = c(confusionMatrix(table(classifications, trainingDataSet$Attrition))$overall[1], confusionMatrix(table(classifications, trainingDataSet$Attrition))$byClass[2], k, paste(c(3,4,5,6), collapse = " "))
      }
      resultsDataframe[nrow(resultsDataframe) + 1,] = c(mean(as.numeric(iterationResultsDataframe$accuracy)), mean(as.numeric(iterationResultsDataframe$specificity)), k, paste(c(3,4,5,6), collapse = " "))
    }
  #}
  #fileLocation = paste("C:/SMU MS Data Science/DoingDataScience6306/FinalProject/KnnResults/Combinations2/Results", i, ".csv", sep="")
  #write.csv(resultsDataframe, fileLocation, row.names = TRUE)
...
```{r}
#turnoverDataframe$Attrition
numberOfYesObservations = nrow(turnoverDataframe[turnoverDataframe$Attrition == "Yes",])
numberOfNoObservations = nrow(turnoverDataframe[turnoverDataframe$Attrition == "No",])

numberOfYesObservations
numberOfNoObservations
```
