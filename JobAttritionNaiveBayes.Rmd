...
```{r}
trainIndices = sample(seq(1:length(turnoverDataframe$Attrition)),round(.7*length(turnoverDataframe$Attrition)))
trainTurnover = turnoverDataframe[trainIndices,]
testTurnover = turnoverDataframe[-trainIndices,]

model = naiveBayes(trainTurnover[, c(3, 5, 6)], trainTurnover$Attrition, laplace = 1)
confusionMatrix(table(predict(model, testTurnover[, c(3, 5, 6)]), testTurnover$Attrition))
